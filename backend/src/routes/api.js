...
const SaaSData_1 = __importDefault(require("../models/SaaSData"));
...
router.get('/pricing', async (req, res) => {
    try {
        ...
        const data = await SaaSData_1.default.find(filter).sort({ companyName: 1, price: 1 });
        res.json(data);
    }
    catch (error) {
...
router.get('/analytics', async (req, res) => {
    try {
        const allData = await SaaSData_1.default.find({});
